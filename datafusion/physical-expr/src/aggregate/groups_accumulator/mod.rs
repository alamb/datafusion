...
/// Describes how many rows should be emitted from an accumulator.
#[derive(Debug, Clone, Copy)]
pub enum EmitTo {
    /// Emit all groups
    All,
    /// Emit only the first `n` groups and shift all existing group
    /// indexes down by n. For example, if `n=10`, group_index `0, 1,
    /// ... 9` are emitted and group indexes '`10, 11, 12, ...` become
    /// `0, 1, 2, ...`.
    First(usize),
}

impl EmitTo {
    /// Does self specify something other than `All`?1
    pub fn is_some(&self) -> bool {
        matches!(self, Self::First(_))
    }

    /// Removes the required number of rows from `v`, returning a Vec
    /// with elements taken, and the remaining values in `v`.
    ///
...
pub trait GroupsAccumulator: Send {
    ...
    /// `merge_batch` will be shifted by `first`. For example, if
    /// emitting the first `10` groups, the first 10 rows should be
    /// returned and every remaining group's group_index is lowered by
    /// 10. Data for group_index `17` will be come the data for
    /// group_index `7`, and so on.
